...
void ship::attack(enemyShip& enemy){
    ...
    if (distance <= maxRange) {
        ...
        if (enemy.health <= 0) {
            ...
            enemy.defeat_counter += 1;
            int num1, num2;
            do {
                num1 = rand() % 2;
                num2 = rand() % 2;
                enemy.x = num1 * (rand() % 10 + 1);
                enemy.y = num2 * (rand() % 10 + 1);
            } while (enemy.ally->x == enemy.x && enemy.ally->y == enemy.y);
            enemy.health = 50;
        } else {
...
void printmap(ship& ship, enemyShip enemy){
    cout << endl;
    for (int i = -10; i < 10; i++) {
        for (int j = -10; j < 10; j++) {
...
int main(int argc, char const *argv[])
{
    ...
    // Game over
    Sleep(1000);
    cout << "Kapal tenggelam! Jumlah kapal musuh yang berhasil dikalahkan: "<< enemy.defeat_counter << endl;
    return 0;
}
